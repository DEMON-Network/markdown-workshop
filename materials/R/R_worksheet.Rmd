...
1. Try to create the Markdown script for this html output (you are welcome to copy the r code).
2. This Markdown is still very confusing. Try to make this Markdown script clearer (example: Reproducible-Reporting-using-Markdown-R/RStudio_advancedMarkdown.html)
    + hide code
    + create a table of content that links to the sections
    + delete the hashtags at the beginning of the output of the "Variable Types", the "Simple Linear Regression" and the Reference section
    + reduce histograms from "Visualize Data" section to 50% width and place them next to each other
    + reduce simple linear regression plots to 70% width
    + create a results table for the multiple regression including N, confidence interval and r-squared values  
...
```{r stratified histograms, message = FALSE}
...
```{r simple linear regression CDR strat, message=FALSE}
plotAgeAssociation <- function(xaxis) {
  # create complete cases dataset
  df.plot <- df %>%
    dplyr::select(MMSE, as.name(xaxis), CDR) %>%
